...
describe('prompt library store', () => {
  ...
  it('updates an existing prompt through edit mode', () => {
    ...
    store.getState().actions.startEdit(promptToEdit.id)
    store.getState().actions.updateDraft('title', 'Updated Prompt')
  ...
  it('duplicates and deletes prompts while keeping selection valid', () => {
    ...
    const duplicate = store.getState().actions.duplicatePrompt(sourcePrompt.id)

    expect(duplicate?.title).toBe(`${sourcePrompt.title} (copy)`)
    expect(store.getState().selectedPromptId).toBe(duplicate?.id ?? null)

    const deletedPrompt = store
      .getState()
      .actions.deletePrompt(duplicate?.id ?? '', sourcePrompt.id)
